class Game {
  constructor(context, canvas) {
    ...
    this.circle = new Circle(200, 200, 20, 5, 5);
    this.bricks = [];
    this.bricks2 = [];
    this.gameOver = false;
    this.points = 0;
  ...
  _bounceWalls() {
    ...
    if (
      this.circle.x + this.circle.size > 1000 ||
  ...
  _spaceBarCollision() {
    if (
      this.circle.y + this.circle.size > 600 ||
      (this.circle.y + this.circle.size > this.spaceBar.y &&
        this.circle.x > this.spaceBar.x &&
        this.circle.x < this.spaceBar.x + this.spaceBar.width) ||
  ...
  _checkCollision() {
    this.bricks.forEach((elem) => {
      if (
        this.circle.x > elem.x &&
        this.circle.x <= elem.x + elem.width &&
        this.circle.y > elem.y &&
        this.circle.y < elem.y + elem.height
      ) {
        brickHit.play();
